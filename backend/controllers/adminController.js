...
const registerAdmin = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Error in Register to admin" });
  }
};
...
const logoutAdmin = async (req, res) => {
  res.clearCookie("admin").status(200).json({
    success: true,
...
const addDoctorByAdmin = async (req, res) => {
  ...
  try {
    ...
    return res.json({
      success: false,
      message: error.message + " in catch block of doctor added",
    });
  }
};
...
const updateDoctorProfileByAdmin = async (req, res) => {
  ...
  try {
    ...
    if (req.file) {
      ...
      if (imageTempPath) {
        try {
          ...
          // console.log( "this is cloudinary response", cloudinaryResponse,);
          if (!cloudinaryResponse || cloudinaryResponse.error) {
    ...
    const updatedDoctor = await doctorModel.findOneAndUpdate(
      { doctorEmailId: doctorEmailId }, // filter by doctorEmailId
      doctorData, // the data to update
  ...
  try {
    if (req.file) {
      ...
      for (let i = 0; i < response.length; i++) {
        ...
        if (existingDoctor) {
          console.log(
            `Doctor with email ${response[i].email} already exists. Skipping...`
          );
          continue; // Skip adding this doctor if already exists
        }

        // Store plain password for email notification before hashing
        const plainPassword = response[i].password;
        ...
        doctorData.push({
          ...
          doctorMeetLink: response[i].doctorMeetLink,
...
const sendForgotPasswordCode = async (req, res) => {
  ...
  try {
    ...
    let info = await transport.sendMail({
      ...
      subject: "Rana Hospital : Password Reset Code",
      html: `
    <div style="font-family: Arial, sans-serif; color: #333;">
      <div style="max-width: 600px; margin: 20px auto; padding: 20px; border: 1px solid #eaeaea; border-radius: 8px; background-color: #1c1c1c; color: #f4f4f4;">
        <h2 style="color: #00ccff; text-align: center;">Rana Hospital</h2>
        ...
        <div style="border-top: 1px solid #eaeaea; margin-top: 20px; padding-top: 10px;">
          <p style="font-size: 12px; text-align: center; color: #999;">
            &copy; ${new Date().getFullYear()} Rana Hospital. All rights reserved.
          </p>
        </div>
      </div>
    </div>
...
const getAllDiagnosesByAppointmentId = async (req, res) => {
  try {
    // Extract the appointmentId from the request query
    const { appointmentId } = req.query;

    console.log("Fetching diagnoses for appointment ID:", appointmentId);

    // Fetch the diagnoses using the appointmentId, populating the related data
    ...
    // If no diagnoses found, return a 404 response
    ...
    // Map and return the fetched diagnoses with detailed information
    res.status(200).json({
      success: true,
      diagnoses: diagnoses.map((diagnosis) => ({
        ...
        residence: diagnosis.residence,
        family: diagnosis.family,
        membersInFamily: diagnosis.membersInFamily,

        // Medical details
        diagnosis: diagnosis.diagnosis,
        symptoms: diagnosis.symptoms,
        prescriptions: diagnosis.prescriptions.map(prescription => ({
        ...
        dietaryRestrictions: diagnosis.dietaryRestrictions,
        followUpDate: diagnosis.followUpDate,
        labTests: diagnosis.labTests.map(test => ({
        ...
        reportPdfUrl: diagnosis.reportPdfUrl,

        // Related information
        doctorDetails: {
          id: diagnosis.doctorId._id,
          // Add other doctor fields you want to expose
        },

        appointmentDetails: {
          id: diagnosis.appointmentId._id,
          // Add other appointment fields you want to expose
        },
...
export {
  ...
  getSingleDoctor,
  getAllAppointments,
  getAllDiagnosesByAppointmentId,
};
